...
public:
  ...
  bool findNextRemainingComponentOf(StackLevel &top) {
    ...
    // if(!config_.perform_thresholding) assert(!top.branch_found_unsat());
    ...
    if (!config_.ssat_solving)
      ...
      top.includeSatProb(1);
      // if(config_.strategy_generation){
      //   Node* n = top.getNode();
      //   if(n->isExist()){
      //     cout << "Mark Max Branch " << top.maxProbBranch() << endl;
      //     n->markMaxBranch(top.maxProbBranch());
      //   }
      // }
    }
    return false;
...
private:
  ...
  // NOTE for SSAT, to perform pure literal detection
  // we have the list  0 binarylinks_pos 0 binarylinks_neg 0 occurence_pos 0 occurence_neg 0
  // for occurence phase lookup, we use another occurence_set
  // len = number of occurence in long clause
  vector<int> unified_variable_links_lists_pool_;
  ...
  // NOTE
  int *beginOfLinkList(VariableIndex v) {
  ...
  //NOTE SSAT pure literal detection
  // remove the occurence clause search state of the pure literal
  // reset the search state the this variable
  // pure phase = phase
  void pureEliminate(VariableIndex var, vector<ClauseOfs>& occur);
  void pureEliminate(VariableIndex var, int* start_ofs);

  //NOTE SSAT pure component, resort to external SAT solver
  bool satSolve(Component* pure_comp);
};
