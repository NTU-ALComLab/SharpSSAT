...
bool Solver<TProb>::simplePreProcess() {
  ...
  if (succeeded) {
    if (config_.ssat_solving) {
      // FIXME
      TProb root_path_prob = stack_.top().getCurPathProb();
      LiteralIndexedVector<TriValue> lv(this->literal_values_);
      HardWireAndCompact();
      this->literal_values_ = lv;
      stack_.top().includePathProb(root_path_prob);
      if (config_.strategy_generation || config_.compile_DNNF ||
          config_.certificate_generation) {
    ...
    if (config_.ssat_solving && config_.strategy_generation &&
        config_.include_forall)
...
bool Solver<TProb>::containUniversalClause() {
  ...
  for (LiteralID l(1, false); l != this->literals_.end_lit(); l.inc()) {
    if (this->qType(l) != UNIVERSAL)
      continue;
    // BEGIN Propagate Bin Clauses
    for (auto bt = this->literal(l).binary_links_.begin(); *bt != SENTINEL_LIT;
         bt++) {
  ...
  for (auto it_lit = this->literal_pool_.begin();
       it_lit != this->literal_pool_.end(); it_lit++) {
    if (*it_lit == SENTINEL_LIT) {
      ...
      if (this->qType(*it_lit) != UNIVERSAL)
        all_universal = false;
    }
  }
  if (!config_.quiet)
...
void Solver<TProb>::HardWireAndCompact() {
  ...
  for (auto l = LiteralID(1, false); l != this->literals_.end_lit(); l.inc()) {
    this->literal(l).activity_score_ =
        this->literal(l).binary_links_.size() - 1;
    this->literal(l).activity_score_ += this->occurrence_lists_[l].size();
  }

  this->statistics_.num_unit_clauses_ = this->unit_clauses_.size();

  this->statistics_.num_original_binary_clauses_ =
      this->statistics_.num_binary_clauses_;
  this->statistics_.num_original_unit_clauses_ =
      this->statistics_.num_unit_clauses_ = this->unit_clauses_.size();
...
bool Solver<TProb>::solve(const string& file_name) {
  ...
  if (!this->createfromFile(file_name))
    return false;
  initStack(this->num_variables());
  if (config_.strategy_generation || config_.compile_DNNF ||
      config_.certificate_generation)
  ...
  if (notfoundUNSAT) {
    if (this->num_variables() == 0) {
      ...
      last_ccl_deletion_time_ = last_ccl_cleanup_time_ =
          this->statistics_.getTime();

      state_.violated_clause.reserve(this->num_variables());
      component_analyzer_.initialize(this->literals_, this->literal_pool_,
                                     this->var2Q_);

      this->statistics_.exit_state_ =
          config_.ssat_solving ? countSSAT() : countSAT();

      if (config_.ssat_solving) {
        this->statistics_.set_final_solution_prob(
            assert_prob_ * stack_.top().getTotalSatProb());
      } else {
        this->statistics_.set_final_solution_count(
            stack_.top().getTotalModelCount());
      }
      this->statistics_.num_long_conflict_clauses_ =
          this->num_conflict_clauses();
...
void Solver<TProb>::decideLiteral() {
  ...
  stack_.push_back(StackLevel<TProb>(
      stack_.top().currentRemainingComponent(), literal_stack_.size(),
...
bool Solver<TProb>::ssatDecideLiteral() {
  // establish another decision stack level
  // cout << "New Stack " << stack_.size() << ", Comp " <<
  // stack_.top().currentRemainingComponent() << endl;
  stack_.push_back(StackLevel<TProb>(
      stack_.top().currentRemainingComponent(), literal_stack_.size(),
      component_analyzer_.component_stack_size()));
  ...
  for (auto it = component_analyzer_.superComponentOf(stack_.top()).varsBegin();
    ...
    assert(this->variables_[*it].component_level ==
           stack_.get_decision_level());
    if (!this->isActive(LiteralID(*it, false)))
      continue;
    score = scoreOf(*it);
  ...
  if (config_.strategy_generation || config_.compile_DNNF ||
      config_.certificate_generation) {
    Node* n = new Node();
    n->setDecVar(theLit.var(), this->qType(theLit) == RANDOM,
                 this->qType(theLit) == UNIVERSAL, theLit.sign());
...
retStateT Solver<TProb>::backtrack() {
  ...
  do {
    ...
    // Force explore both branch if compile_DNNF is true and pure literal is not
    // enabled
    if (!stack_.top().isSecondBranch() &&
        (stack_.top().needSecondBranch() ||
         (config_.compile_DNNF && !config_.perform_pure_literal))) {
    ...
    if (!stack_.top().isSecondBranch() &&
        (config_.compile_DNNF || config_.certificate_generation)) {
      // assert(false);
      Node* node = stack_.top().getNode();
      node->changeBranch();
    ...
    if (config_.ssat_solving) {
      ...
      if (config_.strategy_generation || config_.compile_DNNF ||
        ...
        component_analyzer_.cacheSatProbOf(stack_.top().super_component(), p,
                                           n);
      } else
        component_analyzer_.cacheSatProbOf(stack_.top().super_component(), p,
                                           nullptr);
    ...
    assert(stack_.top().remaining_components_ofs() <
           component_analyzer_.component_stack_size() + 1);
...
retStateT Solver<TProb>::resolveConflict() {
  ...
  if (uip_clauses_.back().front() == TOS_decLit().neg() &&
      config_.perform_clause_learning) {
    assert(TOS_decLit().neg() == uip_clauses_.back()[0]);
    this->var(TOS_decLit().neg()).ante =
        this->addUIPConflictClause(uip_clauses_.back());
...
bool Solver<TProb>::bcp() {
  ...
  for (auto lit : this->unit_clauses_) {
    ...
    if (this->var(lit).component_level != stack_.get_decision_level())
      continue;
  ...
  if (!bSucceeded) {
    ...
    if (this->statistics_.num_clauses_learned_ - last_ccl_cleanup_time_ >
        100000) {
...
bool Solver<TProb>::BCP(unsigned start_at_stack_ofs) {
  for (unsigned int i = start_at_stack_ofs; i < literal_stack_.size(); i++) {
    ...
    // BEGIN Propagate Bin Clauses
    for (auto bt = this->literal(unLit).binary_links_.begin();
         *bt != SENTINEL_LIT; bt++) {
      if (this->var(*bt).component_level != stack_.get_decision_level())
        continue;
    ...
    for (auto itcl = this->literal(unLit).watch_list_.rbegin();
      ...
      if (this->var(*p_otherLit).component_level != stack_.get_decision_level())
        continue;
      ...
      if (*itL != SENTINEL_LIT) {
        ...
        *itcl = this->literal(unLit).watch_list_.back();
        ...
        if (this->isActive(*p_otherLit) &&
            this->qType(*p_otherLit) ==
                UNIVERSAL) { // active universal otherLit
          if (config_.strategy_generation)
...
void Solver<TProb>::printOnlineStats() {
  ...
  cout << "/" << this->statistics_.num_binary_conflict_clauses_ << "/"
       << this->unit_clauses_.size() << endl
       << endl;

  cout << "cache size " << component_analyzer_.cache().used_memory_MB() << "MB"
       << endl;
  cout << "components (stored / hits) \t\t"
       << this->statistics_.cached_component_count() << "/"
       << this->statistics_.cache_hits() << endl;
...
template <typename TProb>
void Solver<TProb>::generateCertificate(const string& up, const string& low,
                                        const string& prob) {
  ofstream out(up);
