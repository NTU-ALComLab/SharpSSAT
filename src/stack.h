...
public:
  ...
  bool needSecondBranch(){
    ...
    if(isR_) return true;
    if( active_branch_==0 &&  (branch_sat_prob_[0]*path_prob_[0])==double(1) ) return false;    // laurenl: on first exist branch, prob=1
    if( active_branch_==1 && ((branch_sat_prob_[0]*path_prob_[0]) >=
                              (branch_sat_prob_[1]*path_prob_[1])) ){                           // laurenl: on second exist branch, existential early return
      return false;
    }
    return true;
  }
